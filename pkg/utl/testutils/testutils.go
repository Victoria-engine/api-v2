...
import (
	...
	"github.com/Victoria-engine/api-v2/pkg/services"
	"github.com/Victoria-engine/api-v2/pkg/utl/middleware"
...
// AuthenticatedRequest : Makes an jwtauth authenticated request for protected routes
func AuthenticatedRequest(h http.HandlerFunc, method, url string, urlParams map[string]string, body io.Reader) *httptest.ResponseRecorder {
	...
	handler := http.HandlerFunc(
		middleware.SetMiddlewareJSON(
			middleware.SetMiddlewareAuthentication(h),
		),
	)
...
func Request(h http.HandlerFunc, method, url string, urlParams map[string]string, body io.Reader) *httptest.ResponseRecorder {
	...
	handler := http.HandlerFunc(middleware.SetMiddlewareJSON(h))
	handler.ServeHTTP(responseRecorder, req)
